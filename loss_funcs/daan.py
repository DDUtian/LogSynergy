...
class DAANLoss(AdversarialLoss, LambdaSheduler):
    def __init__(self, num_class,
                 discriminator_input_dim=768,
                 gamma=1.0, max_iter=1000, **kwargs):
        super(DAANLoss, self).__init__(gamma=gamma, max_iter=max_iter, discriminator_input_dim=discriminator_input_dim, **kwargs)
        self.num_class = num_class
        self.local_classifiers = torch.nn.ModuleList()
        for _ in range(num_class):
            self.local_classifiers.append(Discriminator(input_dim=discriminator_input_dim))
    ...
    def get_local_adversarial_result(self, x, logits, source=True, lamb=1.0):
        loss_fn = nn.BCELoss()
